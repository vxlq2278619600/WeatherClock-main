...
static const char *weekday_to_string(uint8_t weekday)
{
    switch (weekday)
    {
    case 1: return "Mon";
...
void main_page_display(void)
{
    ...
    do {
        ...
        ui_write_string(35, 121, "----/--/-- Thu", mkcolor(143, 143, 143), color_bg_time, &font20_maple_bold);
    } while (0);

    do {
        ui_fill_color(15, 165, 114, 304, color_bg_inner);
        ui_write_string(19, 170, "Indoor", mkcolor(0, 0, 0), color_bg_inner, &font24_maple_semibold);
    ...
    do {
        ...
        main_page_redraw_outdoor_city("Hefei");
        main_page_redraw_outdoor_weather("--");
...
{
    char str[18];
    snprintf(str, sizeof(str), "%04u/%02u/%02u %s",
        date->year, date->month, date->day, weekday_to_string(date->weekday));
...
{
    ui_fill_color(127, 170, 223, 188, color_bg_outdoor);
    ui_write_string(127, 170, city, mkcolor(0, 0, 0), color_bg_outdoor, &font24_maple_semibold);
}

void main_page_redraw_outdoor_weather(const char *weather)
{
    ui_fill_color(127, 219, 223, 236, color_bg_outdoor);
    ui_write_string(127, 220, weather, mkcolor(80, 42, 24), color_bg_outdoor, &font16_maple);
}
